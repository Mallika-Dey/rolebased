...
public class CustomAuthorizationFilter extends OncePerRequestFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthenticationToken(String header) {
        if(header != null){
            ...
            List<GrantedAuthority> authorities = JWTUtils.hasTokenExpired(token)? null : JWTUtils.extractRoles(token);
            
            if(user!=null) return new UsernamePasswordAuthenticationToken(user,null,authorities);
        }
        return null;
    }
}
